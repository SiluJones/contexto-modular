...
check("G4 switch HUB round-trip (dev: no->sem / yes->com)", () => {
  ...
  assert(noHub !== yesHub, "round-trip nao alterou o CLAUDE.md");
  return "ok";
});

// ============ POR NICHO (17) ============
const COMP = "Princípios universais (definição completa no CLAUDE.md)";
ids.forEach(id => {
  check("N["+id+"] Instr+CLAUDE, teto 6500, universais comprimidos, sem undefined, IDEAS/HUB, chips", () => {
    ...
    assert(cmd && cmd.length > 1000, "CLAUDE.md vazio/curto");
    ...
    assert(!/undefined/.test(cmd), "'undefined' no CLAUDE.md");
...
check("C1 D-018 mount (upload direto + conector do GitHub) no CLAUDE.md", () => {
