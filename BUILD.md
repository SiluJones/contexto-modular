...
src/
  index.template.html <- o "casco": HTML + CSS + JS de núcleo, com 18 marcadores //__KCU_NICHE:<id>__//
  niches/
    dev.js ... custom.js   <- 18 módulos de dados (cada um = objeto NICHES.<id> = { ... };)
```
...
## Ciclo de uma WO (com o portão read-only)
`/check-wo <arquivo>` → `/apply-wo <arquivo>` → `node build.js` → `node validate.js index.html` → `/wrap` → commit.
O `/check-wo` (i-N39, renomeado na wo0053) é **read-only**: confere âncoras, pré-requisitos e o estado verde do repo **antes** de você mexer — não edita, não builda, não commita. Serve pra pegar âncora morta e colisão antes que custem.
