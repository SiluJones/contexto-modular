...
## D-115 — O `.claude/settings.json` gerado é JSON válido; `Write` entra no allow; o kit do Code ganha a ordem do push; `CONTINUIDADE.md` deixa de ser rotulada como snapshot (wo0081)

**Base.** `meta/analises/260807-ANALISE-feedback-dois-narrativos.md` (leva 1) + a nota `260805-0834.txt` + a resposta do autor de 2026-08-07. Defeitos reportados pelo projeto *My Little Lady* em 2026-07-23.

**Quatro defeitos medidos no mesmo mount (`_MANIFEST` 2026-08-07 06:15, commit `315d4bd`).** (1) `src/index.template.html` emitia o `settings.json` do kit-Code com uma linha `//` **depois** do `}` final — JSON inválido, e o Claude Code descarta o arquivo inteiro em silêncio: caem juntos `allow` e `additionalDirectories`, a chave de que dependem o relatório em arquivo (D-108) e a medição delegada (D-113). (2) `Write` não estava no `allow`, e a skill `/wrap` do próprio kit manda criar `logs/AAAA-MM-DD.md` — a skill pedia o que a permissão negava. (3) `CONTINUIDADE.md` era rotulada `Snapshot` na tabela de documentos do CEREBRO gerado, contra o próprio nicho narrativo, que declara o arquivo como *"A memória factual. CRESCE."*. (4) o kit do Code não dizia nada sobre push — nem quando empurrar, nem em que ordem —, e isso produziu relatórios afirmando "push pendente" enquanto o push saía logo depois, sem registro (`260803-1000.txt`, `260805-0834.txt`): a ordem nunca tinha sido escrita, então o relatório mentia por construção. Nenhum dos 80 checks pegava o defeito (1) porque o harness nunca abria um `.json` pelo parser (`grep -c "JSON.parse" validate.js` = 0): o único check do arquivo testava por substring `/"permissions"/` e `/"deny"/`, que casam num arquivo quebrado — a mesma doença da D-110 numa superfície acima.

**Decisão.** Nove edições em `src/index.template.html` + o check novo: (a) o comentário sai de dentro do array JSON do settings gerado; (b) `Write` entra no `allow`; (c) o lembrete de build/teste não se perde — já existe em dois lugares corretos (`CLAUDE.md` gerado e tabela do CEREBRO); (d) `fileBehaviorLabel` passa a testar `CONTINUIDADE` e `BRIEF` separadamente — CONTINUIDADE devolve "Cresce (memória factual, append-only)", BRIEF continua "Snapshot"; (e) o `CLAUDE.md` do kit-Code ganha a seção «Push e relatório — nesta ordem, sempre»: **verde** (validação passou, ou WO só de doc com `git diff` conferido) empurra sozinho, sem perguntar; **vermelho** (validação falhou, âncora não encontrada, `git diff` fora do previsto) não commita nem empurra e fecha com **menu numerado** de saídas reais (a recomendada em 1) — nunca pergunta em prosa, porque pergunta escrita no meio do texto passa despercebida; o relatório é sempre o **último passo**, escrito depois da decisão de push, e se a escolha do menu chegar depois, o relatório se reescreve, não fica valendo a versão velha; (f) as skills `apply-wo` e `wrap` do kit-Code e o modelo de WO (`buildWoTemplate`) ganham a mesma ordem; (g) o pacote de update alcança quem **já instalou** o arquivo quebrado — consertar o gerador não conserta o instalado, e o `role` do `.claude/settings.json` no pacote ganha `CORRECAO OBRIGATORIA`: apague a linha `//` se ela existir, confira que `Write` está no `allow`.

**Check C37 novo e o princípio que ele instala.** Todo artefato emitido com extensão de formato conhecido é aberto pelo **parser daquele formato**, nunca por substring — o check parseia com `JSON.parse` tanto o `settings.json` do kit-Code quanto o do pacote de update, e falha nomeando a causa (`// ou vírgula sobrando quebra o arquivo inteiro e derruba TODAS as permissões em silêncio`) em vez de só acusar sintoma. `fileBehaviorLabel` precisou entrar no `SHIM` do harness para o check chamá-la diretamente.

**Por quê.** Duas frentes recentes (D-108, relatório em arquivo; D-113, medição delegada) foram construídas em cima de uma permissão que, no arquivo efetivamente entregue, nunca chegou a existir — `additionalDirectories` estava presente no código-fonte do gerador, mas o JSON malformado apagava o array inteiro no momento em que o Claude Code tentava lê-lo. Um harness que testa o que gera pelo texto, e nunca pelo leitor do formato, não pega esse tipo de falha — é a mesma lição da D-110 (higiene do bloco gerado) numa camada diferente: o formato de saída importa tanto quanto o conteúdo.

`KIT_VERSION 1.102.0`. Check **C37** novo. **Custo de teto: zero** — C28 imprime os mesmos números da v1.101.0 (`padrao 6618/6900 · +Code 522/550 · +ASU 372/400 · compart 372/450 · combo 7512/7600`). Harness **18/18, 80/80 → 81/81, 0 erros**.

---

## D-116 — O protocolo de update é aditivo (o merge só compara o que é novo); registro `REVOCATIONS` publicado no manifesto do pacote + carimbo de modos completo com a regra de reportar e não remover; quatro regras de higiene universais nos 18 CEREBROs; gatilho do bloco de fecho entra na tabela de eventos (wo0082)

**Base.** `meta/analises/260807-ANALISE-feedback-dois-narrativos.md` (leva 3) e `meta/analises/260807-ANALISE-extracao-de-acervo-nicho-narrativo.md` (leva 4a), fundidas por decisão do autor em `260807-1701.txt`. Evidência: repos de *My Little Lady* (`44eaf44`) e *I will die* (`ea086dc`).

**O achado.** O protocolo de update do kit sabe somar e não sabe subtrair. O merge compara o que é *novo* no template; uma correção que **apaga** uma linha nunca atravessa, porque não há nada novo para comparar. A D-102 protege arquivo vivo refinado — e a proteção funcionou como projetada —, mas ela não tem como expressar «esta linha foi removida de propósito; remova a sua também». Medido: o `meta/CEREBRO.md` do *My Little Lady* mandava relistar o mount *«sempre que o autor sinalizar upload»*, texto apagado pelo kit na **v1.90.0** (wo0068/D-102) com o diagnóstico de que a lista de sinais ensinava a **esperar** o sinal. O projeto integrou a **v1.96.0** — seis versões depois — e ficou com a linha errada; a consequência (mount só relistado quando o autor mandava) foi registrada lá como FIX-016 e diagnosticada como falha de adesão do assistente. **Não era: era o CEREBRO deles mandando fazer aquilo.**

**Duas superfícies do mesmo defeito.** (1) Linha de comportamento apagada de propósito que não atravessa o merge, porque o merge só aponta novidade. (2) Quando um projeto **desliga um modo** (o *My Little Lady* migrou de ASU para Code em 2026-07-23), as seções que aquele modo emitiu ficam no CEREBRO **já gerado** — um CEREBRO gerado é arquivo, não função, e o kit não tem como saber qual configuração o projeto usou quando gerou o arquivo dele.

**Decisão — duas saídas correspondentes.** (a) Registro `REVOCATIONS` publicado na seção «Linhas revogadas» do `_UPDATE-MANIFEST.md`: cada entrada leva o texto antigo citável (para o merge/usuário procurar por substring) e o motivo, para quem receber decidir entre remover ou registrar desvio. (b) Carimbo de modos do manifesto passa a declarar os **quatro** modos (skills, Code, ASU, compartilhado) — hoje só declarava dois, omitindo justamente ASU e compartilhado, os dois que geraram sobra em campo — acompanhado da regra de **reportar como choque e nunca remover sozinho**: o kit não distingue «sobra de configuração antiga» de «modo esquecido ao gerar o pacote», então a decisão fica sempre com o usuário.

**Limite do registro (colisão encontrada ao implementar).** A primeira versão incluía a migração `spec` → WO e **derrubou o C15**, que proíbe a string do caminho antigo no produto. Confirmou o limite certo: **só linha de COMPORTAMENTO apagada entra** no `REVOCATIONS`; migração de formato já é coberta pela cláusula de formato descontinuado, e citar o nome velho reintroduziria no produto o termo que outro check proíbe. O autor já tinha levantado a mesma ressalva por outro caminho (risco de um projeto novo confundir com o `SPEC.md` do SDD). O item ficou de fora.

**Quatro regras de higiene universais**, produzidas com custo real nos dois projetos narrativos: (1) **varrer pelo fato, não pela frase** — um turno inteiro perdido lendo derivado como fonte; a regra nomeia as skills como a superfície mais esquecida e mais perigosa, porque são lidas ANTES de trabalhar; (2) **documento derivado nunca é fonte** — onze resíduos em cinco arquivos depois de uma refutação registrada; exige marca no nome do arquivo derivado e conferência no vivo antes de usar um dado dele; (3) **citar a frase-gatilho antes de perguntar** — um autor perto de largar a obra por receber pergunta sem o trecho citado; (4) **mudança de método não se adota no meio do trabalho** — três desenhos de método trocados no meio do trabalho; refinar conteúdo é dever contínuo, trocar COMO o trabalho é feito é troca de trilho e para para virar análise.

**Gatilho do fecho na tabela.** A regra do Bloco de fecho existe desde a wo0058, nas Instruções, mas não estava na **tabela de gatilhos** — a superfície consultada por evento —, e foi por isso que ela não rodou por uma conversa inteira num projeto real. Nova linha em `TRIGGERS_BASE`: «Fim de QUALQUER turno de trabalho» → emite o Bloco de fecho, sem esperar fim de conversa.

**Check C38 novo.** Confere as quatro regras nos 18 CEREBROs gerados, a linha do gatilho na tabela, a forma do registro `REVOCATIONS` (versão semver, texto citável ≥12 chars, motivo ≥40 chars, sem migração de formato), e que o manifesto do pacote — gerado com Code ligado e ASU/compartilhado desligados — traz a seção «Linhas revogadas», declara `ASU nao`, declara o modo compartilhado, e proíbe remover sozinho.

`KIT_VERSION 1.103.0`. Check **C38** novo. **Custo de teto: zero** — C28 imprime os mesmos números da v1.102.0 (`padrao 6618/6900 · +Code 522/550 · +ASU 372/400 · compart 372/450 · combo 7512/7600`), porque tudo entra em CEREBRO e manifesto, nada nas Instruções. Harness **18/18, 81/81 → 82/82, 0 erros**.

---

## D-117 — Quinta skill do pacote narrativo: `ficha-de-choque`, para extração de acervo pré-existente e choque de continuidade (wo0083)

**Base.** `meta/analises/260807-ANALISE-extracao-de-acervo-nicho-narrativo.md` (leva 4b), decisões do autor em `260807-0802.txt` e `260807-1701.txt`. Fontes primárias: `.claude/skills/ficha-de-choque/SKILL.md` e `DECISIONS.md` de *I will die before the game story start* (`ea086dc`); `meta/CEREBRO.md` §«Worldbuilding: extração em dois turnos» e §«Como apresentar extração e choque (DEC-033)» e `DECISIONS.md` de *My Little Lady* (`44eaf44`).

**O achado.** O nicho `narrative` modela **fundar** uma obra e **escrevê-la**, mas não modela **herdá-la**: `grep "extra[cç]"` em `src/niches/narrative.js` retornava zero antes desta WO. Dois projetos rodando exatamente essa fase — extrair worldbuilding de acervo antigo para o cânone — inventaram método por conta própria, e os dois métodos se completam em vez de competir. O custo de não ter isso é medido e é alto: num deles o autor chegou perto de largar a obra por receber conflito por referência cruzada nua (`CONTINUIDADE:93`, «ver DEC-025»), como se decorasse o que escreveu meses atrás; no outro, um capítulo inteiro fugiu de regras já registradas no acervo **sem o autor saber que houve choque**, porque contradizer prosa já escrita fazia a ideia ser descartada em silêncio.

**Genealogia honesta.** O protocolo **não** é convergência independente — o autor transplantou os quatro campos de um projeto para o outro. O que o transplante prova é **reprodução**, não invenção dupla: o formato pegou noutro projeto e, no primeiro lote real produzido no projeto receptor, gerou duas regras novas ali, inclusive uma correção de atribuição do autor contra o assistente.

**O que veio de cada lado.** Do *I will die*: os quatro campos com o texto pronto no campo 4 (não a descrição da mudança, a linha exata), status incluindo 🔧 «resolvido sozinho» sob o critério duplo da DEC-8 (nenhuma prosa depende do resultado **e** existe reconciliação sem descartar fonte nenhuma), ciclo de leva, formato de lacuna («O QUE ESTÁ EM ABERTO» quando não há «escrito hoje» para chocar), DEC-11 (extração não é transcrição — o item vem com o que destrava, o que atropela e ao menos uma alternativa) e DEC-12 (refinar o rascunho do autor por padrão, com 2–4 variações). Do *My Little Lady*: as marcas de confiança (literal/síntese/conflito/aberto), a regra da frase-fonte (sem frase citável, o item não pode ser literal), o tratamento de nota antiga como suspeita por padrão, e conferir o cânone antes de abrir pendência.

**O teste de entrada, e por que existe.** Primeira linha do corpo da skill: serve só para obra que **herda um acervo** — notas antigas, planilhas, worldbuilding escrito antes do primeiro capítulo. Obra fundada agora, com o mundo nascendo na conversa, não usa — pergunta em prosa e segue. O autor pediu isso explicitamente em `260807-1701.txt`: ele tem obra sem acervo, e a skill não pode virar marreta em noz.

**O ticket de leva.** Cada leva sai em `AAMMDD-HHMM-leva-<assunto>.md`, fora do versionamento, em `notas-arquivadas/` do lado do projeto (não nasce nesta WO — nasce no primeiro uso). Só o que está em aberto entra; decidido sai na hora para o cânone e não reaparece — acumular verde e vermelho fechados foi o que inutilizou a primeira tentativa disso. Correção registrada: **o ticket novo se deriva do cânone e das respostas do autor, nunca do ticket velho** — o anterior só serve para saber o que perguntar de novo; sem essa regra o ticket vira segunda fonte desatualizada, o defeito exato que ele existe para evitar. Cabeçalho declara que é saída de assistente, não cânone.

**A fronteira, e por que não é muralha.** Ordem de leitura das notas, tamanho da leva e sequenciamento **são do autor**, sempre — a skill não decide isso. A DEC-024 do *My Little Lady* revogou uma mudança de ordem que o próprio assistente propôs («é o que acontece ao ler um documento que responde perguntas que ainda não foram feitas»), e trocar o funcionamento da extração no meio do trabalho já custou três reprojetos seguidos num projeto real, todos pelo mesmo motivo: otimizar o processo em vez de executá-lo. O autor esclareceu que isso não é para virar muralha — propor continua permitido —, então a regra que ficou é de **quando**, não de **se**: trocar de trilho para e vira análise.

**Alcance — só `narrative` por ora.** `game` e `rpg` esperam por pedido do autor: um nicho por vez, porque a skill vai mudar depois da primeira leva real.

`KIT_VERSION 1.104.0`. Check **C39** novo, verde reportando 5 skills no pacote. **Custo de teto: zero** — C28 imprime os mesmos números da v1.103.0 (`padrao 6618/6900 · +Code 522/550 · +ASU 372/400 · compart 372/450 · combo 7512/7600`): a skill vive no CEREBRO gerado e no `skills.zip`, e o gatilho novo entra em `triggersExtra`, que também não passa por `buildInstr`. Harness **18/18, 82/82 → 83/83, 0 erros**.

---

## D-118 — Vocabulário decidido pelo autor (turno x conversa); o prompt de update passa a carregar as linhas revogadas e o carimbo de modos; o modelo de WO entra no próprio `git add` (wo0084)

**Base.** `260811-HANDOFF-BRIEF.md` §3.2 e §3.1. Decisão do autor (2026-08-11).

**Vocabulário.** **Turno** = uma troca de mensagens · **conversa** = o fio inteiro, do início ao fim da sessão de chat. Treze ocorrências trocadas uma a uma, sem `sed` — **não é rename**: o que já estava certo fica como está — `logs/AAAA-MM-DD.md` é da conversa (uma entrada por dia, `## Sessão N` para cada uma), `SESSAO.md` do nicho `rpg` é sessão de mesa (outro sentido), a tabela de quando abrir conversa nova, e as menções a `sessão` fora da lista de Edições 3–7 desta WO.

**Não foi cosmético.** `## Ritual de início de sessão` contradizia a **D-111**, que manda rodar a abertura *antes de QUALQUER outra ferramenta*, **todo turno** — o nome do ritual dizia uma coisa e a regra mandava outra. A linha `Fim de sessão` da tabela de gatilhos vinha colada, sem distinção visual, em `Fim de QUALQUER turno de trabalho` (gatilho do Bloco de fecho, por turno desde a wo0058/wo0082): os dois eventos — fecho de turno e entrega de arquivos completos de fim de conversa — ficavam indistinguíveis para quem lê a tabela. Passam a ser `Início de turno`, `Fim de QUALQUER turno de trabalho` e `Fim da conversa`, três linhas separadas.

**O furo do `buildUpdatePrompt`, achado no primeiro uso em campo** (pacotes de update para *My Little Lady* e *I will die*, 2026-08-08). A wo0082 (D-116) pôs duas seções novas no `_UPDATE-MANIFEST.md` — «Linhas revogadas» e o carimbo de modos — e a regra de usá-las mora no CEREBRO gerado. Mas o CEREBRO de um projeto desatualizado é **justamente o arquivo velho que o update vem consertar**: ele não conhece a regra nova. A única superfície garantida a chegar lá é o **prompt** que o usuário cola na conversa-alvo — e ele não mencionava nem uma seção nem a outra. Ciclo fechado: a instrução para usar o mecanismo morava no arquivo que o mecanismo existe para corrigir. Os dois prompts reais tiveram de ser escritos à mão. Correção: `buildUpdatePrompt` agora abre com as duas seções, **antes** da lista `Arquivos no pacote:` — quem lê de cima para baixo já começa a comparar antes de saber que elas existem, se a ordem for a errada.

**A WO nunca versionava a si mesma.** O bloco `git add` de cada WO listava a WO *anterior*, então a última ficava sempre não rastreada — aconteceu **três vezes seguidas** (wo0081, wo0082, wo0083). Não era descuido de quem aplica: era o modelo ensinando errado. O modelo de WO (`buildWoTemplate`) passa a incluir o próprio caminho no `git add`, com a cláusula de que `add` vazio (WO já versionada) não é erro.

**Armadilha de medição, registrada porque quase enganou.** `grep -o "sess[ãa]o"` no shell devolveu **0** num arquivo que tinha 19 ocorrências — falha de locale em acento. Medição correta via Python (`re.findall` com `encoding=utf-8`).

**Check C40 novo.** Confere, nos 18 nichos: o ritual se chama `Ritual de início de turno` (Instruções e CEREBRO) sem sobra de `sessão`; `lido toda sessão` não aparece; `Ao final da conversa` está no título de entrega de arquivos; a tabela de gatilhos tem `Início de turno` (com a cláusula "todo turno"), `Fim de QUALQUER turno` e `Fim da conversa` como linhas distintas; a recomendação de configuração se declara `fim de turno`. Confere também que `buildUpdatePrompt` cita «Linhas revogadas» e «carimbo de modos» antes de `Arquivos no pacote:`, e que `buildWoTemplate` inclui a própria WO no `git add` com a cláusula de idempotência.
...
## D-139 — O nome do relatório passa a dizer do que ele trata (wo0110)

**Data:** 2026-09-03 (aplicada 2026-09-04) · **Base:** `meta/analises/260903-ANALISE-forma-do-fecho-e-nome-do-relatorio.md`, **Decidida** pelo dono: opção **F com G**. · **Supersede:** o padrão `AAMMDD-HHMM-code-<slug>.txt` instituído pela wo0074/D-119.

**Decisão:** o relatório do Code passa a se chamar `AAMMDD-HHMM-code-<slug>-<ato>-<alvo>.txt`. **`<ato>`** é de **vocabulário FECHADO** — `apply` · `wrap` · `probe` · `explore`, nada fora da lista; **`<alvo>`** é a WO (`wo0110`) ou, sem WO, duas ou três palavras com hífen. **Somar, não trocar:** o `<slug>` fica, porque a razão dele está escrita no próprio kit — *«a pasta-pai costuma ser compartilhada por vários repos»* — e o mapsmith, de quem o padrão foi adotado, o perdeu ao trocar o slug pelo ato. Sem lista fechada, o `<ato>` viraria texto livre e a praticidade acabaria em três meses; os dois casos fora de WO que o dono citou (sonda, exploração) são exatamente os que caíam no nome genérico e agora têm `probe`/`explore`.

**Alcance — oito edições, cinco superfícies do gerador + três instaladas do KCM.** No gerador (`src/index.template.html`): a linha do CLAUDE.md gerado que institui a gravação (agora com o glossário dos três campos e um exemplo), as duas skills (`apply-wo` fixa o ato em `apply`, `wrap` em `wrap`), e a lista de obrigações do `settings.json`. O glob `../AAMMDD-HHMM-code-*.txt` da `wrap` **não muda** — casa com os nomes novos e ordena por commit. Nas superfícies instaladas do próprio KCM: `CLAUDE.md`, `.claude/skills/apply-wo/SKILL.md`, `.claude/skills/wrap/SKILL.md` — porque *ninguém baixa o pacote de update do KCM para atualizar o KCM* (consequência aceita na análise; a `i-N60` já registrou que nenhum check olha para boa parte dessas superfícies).

**Amarrado pelo harness.** O `C30` já ancorava o padrão antigo (`/AAMMDD-HHMM-code-<slug>\.txt/` nas duas skills). Reescrito: passa a exigir o **ato certo em cada skill** (`-apply-<alvo>` em `applyWo`, `-wrap-<alvo>` em `wrap`) e a presença do **vocabulário fechado** no CLAUDE.md gerado — garantia mais forte que a anterior. Continua contando como **um** check; total permanece **100/100**.

**Custo de teto: zero** — as oito edições ficam em `claudeMd`, nas duas skills e no texto do `settings`, nenhuma em `buildInstr`. `index.html` 836.117 → 836.745 bytes. Harness **18/18 · 100/100 · 0 erros**. Par negativo medido no sandbox pela autora da WO (reverter a linha da `apply-wo` deixa o `C30` vermelho).

**Fora de escopo, registrado:** a forma do fecho de turno (opções B, A e a cláusula da frase copiável) é a **wo0111**, de propósito separada — é texto novo em superfície que nenhum check pode observar. E estender o `C43` ao `meta/workorders/_TEMPLATE.md` (`i-N60`): o gatilho dela — «a próxima WO que tocar o `validate.js`» — **disparou** nesta WO, mas a extensão fica fora para não misturar duas mudanças de instrumento no mesmo commit que reescreve o `C30`. A decisão de quando pagar a `i-N60` é do dono.

---

## D-140 — O bloco de fecho de turno para de derivar: condição observável, esqueleto literal e frase copiável (wo0111)

**Data:** 2026-09-03 (aplicada 2026-09-04) · **Base:** `meta/analises/260903-ANALISE-forma-do-fecho-e-nome-do-relatorio.md`, decidida pelo dono: **B + A**, mais a cláusula da frase copiável fornecida no turno seguinte. · **Supersede:** requalifica o `C19` (wo0058/wo0060/wo0064); completa a análise que a D-139 deixou pela metade.

**(B) A omissão de linha deixa de ser julgamento e passa a condição observável.** «Emitindo só as linhas que se aplicam» entregava a decisão ao próprio emissor — o desenho que a D-135 identificou como o que falha. Agora: **Próximo**, **Estado** e **Config recomendada** saem SEMPRE; **Arquivar / Manter** sai quando há qualquer avulso no mount, e a condição se verifica LISTANDO o mount, não lembrando; **Handoff** sai quando algum arquivo foi entregue NESTE turno. O teste é «a condição é falsa?», nunca «me pareceu que não se aplicava».

**(A) Entra um esqueleto literal.** Dezesseis linhas no CEREBRO gerado (e a versão «casa» no `meta/CEREBRO.md`, com as raias Chat/Code nomeadas em vez de `<raia>`), com a condição de cada rótulo colada à direita — mitigação do risco «o esqueleto vira teatro» que a análise levantou: o campo não aparece com «nada a arquivar» dentro porque a condição está na mesma linha do rótulo. A causa que ele ataca: o bloco era **reconstituído de memória** a cada turno, no fim do turno; medido nesta conversa, quatro turnos seguidos, quatro formas diferentes, com o formato fixo escrito no arquivo lido a cada mensagem. «Copie e preencha, não reconstitua.»

**(cláusula do dono) O «Peça no próximo turno» tem de ser COPIÁVEL COMO ESTÁ** — sem lacuna, reticências ou campo para o dono preencher com dado ou medição («medidas são ___», «(...)», «saída do X aqui»). A única lacuna aceitável é escolher entre opções já enumeradas no próprio turno («fase 3: A, B ou C?»); a frase pode vir com a recomendação já dentro, para só confirmar ou trocar. Caso concreto citado pelo dono: o mapsmith pediu «saída do verify aqui» quando a saída já estava no relatório que o Code havia gerado.

**A casa estava atrás do gerado.** O `meta/CEREBRO.md` do KCM tinha a versão curta (wo0058/wo0060/wo0064); o gerado já trazia o carimbo `Base:` no Estado, a lista exaustiva com o terceiro estado «Já arquivado» e a cláusula do «resultado que o usuário saiba produzir» — nada disso existia na casa. As Edições 5–9 fecham a defasagem no mesmo commit, porque ninguém baixa o pacote de update do KCM para atualizar o KCM.

**Amarrado pelo harness — e a análise errou aqui.** Ela previa que a forma do fecho era «texto novo em superfície que nenhum check pode observar». Mas o `C19` **ancorava a frase que sai** (`/só as linhas que se aplicam/` no CEREBRO gerado): sem reescrevê-lo o harness ficaria vermelho. Reescrito, passa a exigir a `CONDIÇÃO OBSERVÁVEL`, as duas condições coladas nos rótulos, a cláusula `COPIÁVEL COMO ESTÁ` e o `Esqueleto — copie e preencha` — 1 assert virou 4, garantia mais forte que a anterior. Continua **um** check; total **100/100**. O que o `C19` **não** garante: que alguém seguiu o bloco — o harness lê o que o kit emite, nunca a conversa.

**Custo de teto: zero** — tudo em `buildClaudeMd` (CEREBRO gerado, que não tem teto); `buildInstr` só carrega a linha de gatilho, não tocada. `index.html` 836.745 → 839.262 bytes. Harness **18/18 · 100/100 · 0 erros**. Par negativo medido pela autora da WO: removendo só a linha do esqueleto, o `C19` fica vermelho.

**Correção de tabela, no mesmo commit:** as Edições 10–11 tiram o duplo parêntese que o recorte de âncora da wo0110 deixou em `.claude/skills/apply-wo/SKILL.md` e `.../wrap/SKILL.md` (`) (pasta-pai do repo)` → `; pasta-pai do repo)`) — erro de quem escreveu a wo0110, previsto no relatório dela como «higiene de uma linha para depois».

**Registrado, fora de escopo:** estender o `C43` ao `meta/workorders/_TEMPLATE.md` (`i-N60`) — o gatilho «a próxima WO que tocar o `validate.js`» **disparou pela segunda vez seguida** (wo0110 e wo0111 tocaram o `validate.js` em commits consecutivos). Um gatilho não-pago que dispara duas vezes está a caminho de virar pendência perpétua; a decisão de quando pagar é do dono.

## D-141 — A exploração ganha gatilho: o kit passa a gerar a skill `sondar`, e o esqueleto de sonda recebe a errata do nome (wo0112)

**Data:** 2026-09-04 · **Base:** `meta/analises/260904-ANALISE-a-exploracao-sem-gatilho.md`, decidida pelo dono: **A, com D depois**. · **Supersede:** nada — cria a terceira skill do kit do Code (`apply-wo`, `wrap`, agora `sondar`); corrige o comentário do esqueleto de sonda em `buildClaudeMd`.

**O CEREBRO ensinava o par sonda/exploração por inteiro, e o kit entregava só duas skills.** A exploração — funil, terceiro estado, as três propriedades do relatório, a regra de nenhuma das duas dar veredito, o esqueleto do relatório — existia como conhecimento e não como comando. O `mapsmith` tinha fabricado a terceira sozinho; a mesma classe de defeito que a wo0111 tratou no fecho (regra que vive só como prosa é reconstituída de memória), só que aqui a consequência é pior: o fecho **derivava**, a exploração simplesmente **não acontecia**.

**A skill `sondar` entra genérica para os 18 nichos**, adotada do `mapsmith` e reescrita em torno do que **todo** projeto do kit tem — o que existe no disco, o que os `meta/` declaram, o instalado contra o gerado, os cruzamentos que a validação não faz —, com `disable-model-invocation: true` como as outras duas. Grava o relatório com o ato `explore`, do vocabulário fechado da wo0110, sem inventar convenção nova. Ganha a seção que o CEREBRO não tinha: **«o que eu olhei e não achei nada»**, obrigatória — o oposto de «achado», e igualmente necessário, porque passada exploratória que sempre acha algo é passada que inventa.

**A errata do nome (achada durante a leitura do `mapsmith`, virou parte da mesma WO por decisão do dono):** o esqueleto de sonda dizia `# SONDA — <assunto> <- o nome diz o QUE, não o quando`, ensinando só a metade do que vale para sonda de assunto único. Sonda **reexecutável**, que existe para comparar antes/depois, é série — carimbo de tempo primeiro (`AAMMDD-HHMM-EXPLORACAO` ao lado de `AAMMDD-HHMM-CONFERENCIA`), porque o que importa é a pasta se ordenar sozinha no tempo e o par ficar visível lado a lado; o assunto vai no título, dentro do arquivo. As duas regras já existiam soltas no kit (o `mapsmith` usa carimbo primeiro há mais de um mês); faltava dizer quando cada uma vale.

**Seis pontos de registro, medidos antes de escrever a WO:** uma skill nova precisa entrar em `buildCodeKitFiles`, no `return`, na tabela de arquivos do CEREBRO, no zip do kit do Code, no zip completo e no **pacote de update** — esquecer o último faz a skill nunca chegar a quem já tem o kit instalado. As nove edições da WO cobrem os seis pontos; a Edição 5b (pacote de update) é a que a própria WO marcou como «a que ninguém lembra».

**Custo de teto: zero, medido, não suposto.** A análise original supunha que o gatilho custaria teto; a WO mediu e mostrou que a skill é arquivo do kit do Code, a errata vive no CEREBRO gerado, e a tabela de gatilhos também é do CEREBRO — consumida em `buildClaudeMd`, não em `buildInstr`. Os cinco números do `C28` ficaram **idênticos** antes e depois (padrão 6605/6900 · +Code 514/550 · +ASU 372/400 · compart 372/450 · combo 7491/7600).

**Check novo, não reescrita:** `C101` confere que o kit gera a skill `sondar` com as cláusulas que a separam da verificação (não conclui, não parte da lista existente, declara o que olhou e estava limpo, grava com o ato `explore`, não abre ordem de trabalho) e que o CEREBRO cita `/sondar` e a errata do nome; o `C43` ganha um assert pontual para a skill instalada na casa. Total **101/101**. Par negativo medido: trocar a cláusula «não parta da lista de checagens que já existe» pelo oposto derruba o `C101` para vermelho.

**Registrado, fora de escopo (a `i-N60`, terceira vez):** generalizar o `C43` para comparar toda superfície instalada com a gerada — decisão do dono, adiada. `index.html` 839.262 → 844.912 bytes.

---

## D-142 — O resumo do fecho para de ensinar a forma errada, e o funil cobre os quatro estados (wo0113)

**Data:** 2026-09-05 · **Base:** exploração `260904-2340-code-kcm-explore-primeira-passada.txt` (candidatos 1 e 2), decidida pelo dono; a `i-N60` e a IDEA-137 ficam para depois. · **Supersede:** requalifica o `C19` (wo0058/wo0060/wo0064/wo0111) e o `C101` (wo0112) com um assert cada; não cria check.

**A wo0111 consertou o lado errado primeiro.** Ela trocou «emitindo só as linhas que se aplicam» por condição observável no CEREBRO, mas o **resumo do fecho nas Instruções** — lido a cada mensagem, não no ritual — ficou intacto dizendo `(só o que se aplica)` e listando os cinco campos separados por «·». Duas coisas erradas: «só o que se aplica» é a mesma cláusula de julgamento com outras palavras, na superfície lida com mais frequência; e o resumo, por listar os campos e parecer completo, **ensina** um fecho em linha corrida e desincentiva a ida ao CEREBRO. Agora o resumo não lista os campos e manda **copiar o esqueleto do CEREBRO**, não reconstituir de memória. Não é hipótese nova: está no `meta/IDEAS.md` do `mapsmith`, seção «Feedback para o Kit», de 2026-08-28, com custo medido de três turnos deformados, e o dono relata a mesma deformação no visualizador. Três projetos, mesma deformação.

**O funil negava commit a dois estados e calava sobre os outros dois.** A frase listava quatro (`exploração → sonda → instrumento → análise → WO`) e dizia que exploração e sonda «não têm commit, não mudam o repositório», com a negativa encostada na palavra «análise». A análise vai para `meta/analises/`, que **é** versionado — e nos últimos quatro ciclos entrou no `git add` duas vezes (wo0110, wo0112) e ficou de fora uma (wo0109). Agora o funil diz que **análise e ordem de trabalho mudam o repositório e vão com commit**, e que a análise entra no `git add` da WO que nasce dela. O `mapsmith` relatou este erro em 2026-08-30, com o mesmo desenrolar.

**Os asserts olham a linha isolada, não o arquivo.** O `C19` ganha cinco asserts sobre a **linha do resumo** nas Instruções (`buildInstr`): nenhuma variante de «se aplica/se aplicam», a frase «copie o esqueleto do CEREBRO» presente, e a lista de campos separados por «·» ausente. Um assert sobre o texto inteiro das Instruções daria verde com a frase errada morando em outra seção. O `C101` ganha um assert sobre a cobertura de commit no funil (`buildClaudeMd`). Continua **um** check cada; total **101/101**.
...
## D-144 — O `/wrap` cita em vez de concluir sobre medição, e o modelo de WO ganha as armadilhas do `grep -c` e do fim de linha (wo0115)

**Data:** 2026-09-07 · **Base:** IDEA-126 do `mapsmith` (dois casos em três ciclos, medidos lá; opção **b** adotada, avaliada pelos próprios autores como a que ataca a causa) + dois desvios de previsão medidos nesta casa no relatório `260907-1524-code-kcm-apply-wo0114.txt`. · **Supersede:** requalifica o `C30` (wo0074) com três asserts; não cria check. Total **101/101**.

**O `1b` compara o que o git responde, e só isso.** Commit, push, arquivo não rastreado: verificáveis contra `git status`/`git log`. Byte, contagem de item e nome de chave, não — e o `mapsmith` registrou duas ocorrências em três ciclos em que o segundo relatório de um ciclo afirmou o oposto do primeiro (um arquivo dito ausente depois de listado com tamanho e data; uma chave dita «não identificada» onze minutos depois de identificada pelo `apply-wo`). O `/wrap` gerado passa a carregar a regra: **sobre medição você não conclui — cita e aponta.** Ou reroda a medição e diz que rerodou, ou escreve «medido no relatório X, não reconferido aqui». Contradizer sem remedir é o modo de falha caro — o log do dia guarda o relatório errado, e desempatar dois registros que se negam custa uma sessão e exige os dois em mãos.

**O KCM não tem ocorrência medida.** Reli um par (wo0114) e o `/wrap` fez o certo: recapitulou o `apply` citando o relatório das 15:24, sem reconcluir. Amostra de um. A evidência é do consumidor e acusa a skill que o kit **gera** — a mesma nos dois projetos. Corrigir o produto por evidência de um consumidor é o caminho normal; esperar a primeira ocorrência na casa seria esperar por um custo já pago em outro lugar.

**As duas armadilhas de contagem entram no modelo de WO, ao lado da regra do número derivado.** Medidas na wo0114, as duas de previsão da autora da WO: (1) `grep -c` conta **linhas**, não ocorrências — em `meta/`, onde um parágrafo é uma linha só, duas menções no mesmo bullet contam 1; para ocorrência, `grep -o ... | wc -l`, e diga qual dos dois previu. (2) Byte previsto carrega o fim de linha — sandbox que grava CRLF erra 1 byte por linha nova quando o arquivo do repo é LF; conferir com `git ls-files --eol` antes de prever tamanho, ou prever faixa em vez de número exato. Não são erros de raciocínio, são erros de instrumento, e o lugar deles é o modelo de WO.

**A casa recebe a mesma regra no mesmo commit.** `.claude/skills/wrap/SKILL.md` ganha o item «cita e aponta» com redação própria (bullets, quebras de linha), e `meta/workorders/_TEMPLATE.md` — entregue pelo chat, 12.093 bytes LF, medido e não suposto — recebe as cinco linhas da armadilha de contagem. O `C43` confere o instalado pela cláusula, não pelo diff, e por isso tolera a diferença de forma. Sem o arquivo do chat no mesmo commit, a casa ficaria atrás do gerado — a classe da `i-N60`.

**Custo de teto: zero.** As três frases moram na skill `wrap` e no modelo de WO — arquivos do kit do Code, fora de `buildInstr`/`buildClaudeMd`. Os cinco números do `C28` idênticos: **padrão 6622/6900 · +Code 514/550 · +ASU 372/400 · compart 372/450 · combo 7508/7600**. `index.html` 845.091 → 846.128 bytes. Par negativo medido: trocar «Sobre MEDIÇÃO você não conclui: cita e aponta» pelo oposto deixa o `C30` vermelho.

**Registrado, fora de escopo:** a `i-N60` (generalizar o `C43`) **disparou pela quinta vez** — esta WO tocou o `validate.js` e ainda substituiu um arquivo instalado que nenhum check cobre; a opção (a) da IDEA-126 (o `1b` listar os achados numéricos do relatório anterior e marcar cada um como confirmado ou não reconferido — mais cara, e os próprios autores avaliam a (b) como a que ataca a causa); a IDEA-137; reler os pares apply/wrap anteriores para medir a frequência na casa (relatórios arquivados, fora do mount). Commit `4e3c2b5`, empurrado (`692f76f..4e3c2b5  main -> main`).

---

## D-145 — O `C43` passa a cobrir as quatro superfícies instaladas da casa: a `i-N60` é paga (wo0116)

**Data:** 2026-09-07 · **Base:** a `i-N60`, aberta na wo0108 (o `meta/workorders/_TEMPLATE.md` instalado ficou 6.618 bytes atrás do gerado por seis WOs, sem nenhum check olhando), com gatilho «a próxima WO que tocar o `validate.js`» que **disparou cinco vezes sem pagamento** — wo0110, wo0111, wo0112, wo0113, wo0115 —, mais a exploração de 2026-09-04 (candidato 3), que mediu o custo e o encolheu. · **Requalifica** o `C43` (wo0087): de duas superfícies instaladas para quatro; não cria check, total **101/101**. · **Fecha:** a `i-N60`, que passa de «ABERTA, com gatilho» a «IMPLEMENTADA na wo0116».

**Gatilho que dispara cinco vezes sem pagamento é pendência perpétua com outro nome.** O `C43` é o único check que abre arquivo do próprio repositório — todos os outros testam o que o kit **emite**. Ele nasceu (wo0087) porque «as skills instaladas ficaram três versões atrás do gerado sem ninguém notar», e cobria duas superfícies (as skills `wrap` e `apply-wo`) mais o `settings.json`. O mesmo defeito reapareceu **no vizinho que ele não cobria**: o modelo de WO instalado. O gatilho para pagar isso foi reportado em cinco WOs seguidas que tocaram o `validate.js` e nunca aplicado. Fica o registro para a próxima ideia com gatilho: **se o gatilho disparar duas vezes seguidas, ou paga ou muda o gatilho.**

**O método NÃO muda — cláusula por regex, não diff.** O candidato 3 da exploração de 04/09 testou a hipótese óbvia (comparar instalado × gerado byte a byte) e a **refutou com medição**: 29 linhas «ausentes» no `CLAUDE.md` e 8 em cada skill, todas falso positivo, porque a casa reescreve as regras com forma própria e substitui os placeholders. Diff literal transformaria adaptação legítima em falha. O caminho foi **ampliar a lista de cláusulas portadoras**, não trocar o instrumento — e foi isso que reduziu o trabalho a uma tabela maior.

**Quatro superfícies, dez cláusulas novas, duas candidatas fora de propósito.** Os mapas `gerado`/`instalado` ganham `woTemplate` (`meta/workorders/_TEMPLATE.md`) e `claudeMd` (`CLAUDE.md`), lidos por `lerRepo`; nasce o mapa `CAMINHO` com os quatro caminhos. Entram seis cláusulas `woTemplate` (âncoras lidas, número derivado, `grep -o … | wc -l`, `git ls-files --eol`, medição prévia, próximo comando) e quatro `claudeMd` (vocabulário de ato, relatório em arquivo, relatório sem pedir, como desligar), cada uma conferida presente **nos dois lados** antes de entrar. **«Config modelo × esforço» e «additionalDirectories» ficaram DE FORA**: existem no gerado e não na casa, que adaptou o `CLAUDE.md` legitimamente — conferi-las seria o erro que a exploração apontou. O `C43` vai de 14 a **24 cláusulas**.

**A mensagem de erro deixa de supor que toda superfície é skill.** O ternário antigo (`alvo==="wrap"?"wrap":"apply-wo"`) devolvia **sempre** um caminho de `.claude/skills/`: com os alvos novos, um `_TEMPLATE.md` desatualizado seria reportado como se fosse `.claude/skills/apply-wo/SKILL.md` — instrumento que relata o fato certo com o lugar errado, a família do FIX-0009 do mapsmith. Passa a usar `CAMINHO[alvo]`. **Dois pares negativos medidos**, um por superfície nova: quebrar `**Ancoras lidas em:**` no `_TEMPLATE.md` e quebrar o vocabulário de ato no `CLAUDE.md` deixam o `C43` vermelho, cada mensagem nomeando o arquivo certo (`meta/workorders/_TEMPLATE.md` e `CLAUDE.md`).

**Custo de teto: não se aplica.** Nada entra em `buildInstr` nem no CEREBRO; os cinco números do `C28` não se movem. **A WO não toca `src/`**: `node build.js` → `index.html` **846.128 bytes**, idêntico antes e depois; `node validate.js index.html` → **18/18 · 101/101 · 0 erros**. `git diff` = `validate.js` + `meta/IDEAS.md` + a WO (nova), aditivo + as três substituições previstas (título do `C43`, comentário da `sondar`, mensagem de erro), sem remoção acidental. Commit `38f1792`, empurrado (`c3b58cd..38f1792  main -> main`).

**Registrado, fora de escopo:** o `meta/CEREBRO.md` da casa (não é superfície instalada — é doc próprio do KCM); o `settings.json` (conferência própria dentro do mesmo check); o carimbo de versão nas superfícies instaladas do KCM (assimetria de propósito, wo0106); as superfícies instaladas nos **outros** projetos (o vetor delas é o pacote de update, parqueado pela D-138); a IDEA-137.

---

## D-146 — A prosa VIVA da casa para de declarar contagem de nicho e caminho de documento que o disco desmente; entra o `C102` (wo0117)

**Data:** 2026-09-09 · **Base:** a exploração que o Code rodou no repositório inteiro (`260908-1009-code-kcm-explore-repo-inteiro.txt`, candidatos 2, 3 e 5) — a metade do terreno que o chat não alcança pelo mount. · **Cria** o `C102` (total **101 → 102**); não muda `src/`, `index.html` idêntico em **846.128 bytes**. · **Não fecha** nenhuma `i-N`: é achado de exploração, não gatilho pendente.

**Cinco pontos de prosa viva estavam atrás do disco, e o `BUILD.md` se contradizia dentro do próprio arquivo.** O `meta/CEREBRO.md` dizia «16 nichos de conteúdo + 1 construtor» (são 17 + 1), «harness dos 17 nichos», «jsdom) dos 17 nichos» e «17/17 nichos» na regra de ouro; o `BUILD.md` dizia `18/18 nichos` na linha 26 e `harness 17/17` na linha 34. A nota de revisão da wo0064 registra que `CLAUDE.md`, `BUILD.md` e `CONTEXT.md` foram corrigidos de 17 para 18 — **o corpo do CEREBRO não foi, e o `BUILD.md` só pela metade**. Ficou assim por 52 versões porque **nenhum check abria esses dois arquivos**: o `grep -nE "readFileSync|lerRepo" validate.js` não tem `meta/CEREBRO.md` nem `BUILD.md` entre os alvos.

**Dois caminhos de documento que o disco desmente.** O CEREBRO citava `` `meta/NICHOS-CANDIDATOS.md` ``, e o arquivo mora na **raiz**; e listava o `DEPLOY-GUIDE.md` entre os arquivos «na raiz», sendo ele entregue **fora do repositório** (o `README.md` e o `CHANGELOG` concordam; o disco não o tem). A correção acompanha a maioria e explicita o «fora do repositório».

**O `17` nem sempre está errado — e isso desenhou o check.** São 18 módulos: 17 nichos de conteúdo + 1 construtor. A frase «17 nichos de conteúdo + 1 construtor» é a ÚNICA forma em que `17` aparece corretamente. Um check que proibisse `17 nichos` daria falso positivo nela. Por isso o `C102` deriva `N` de `build-manifest.json.modules.length` e testa **formas** (`N-1/N-1`, `harness d[oe]s N-1 nichos`, `jsdom) d[oe]s N-1 nichos`), não o número solto; e confere que todo `` `meta/X.md` `` citado na tabela de documentos do CEREBRO existe nesse caminho. Se um nicho entrar, o número acompanha sozinho.

**Dois pares negativos medidos** (sandbox reversível, restaurado depois): devolver `**harness 17/17**` ao `BUILD.md` → **VERMELHO no `C102`**: «BUILD.md declara 17 nichos onde o build-manifest tem 18: a prosa viva ficou atras do disco»; devolver `` | `meta/NICHOS-CANDIDATOS.md` | `` ao CEREBRO → **VERMELHO no `C102`**: «meta/CEREBRO.md cita `meta/NICHOS-CANDIDATOS.md`, que nao existe nesse caminho — documento citado no lugar errado e pior que nao citado».

**Custo de teto: não se aplica** — nada entra em `buildInstr` nem no CEREBRO gerado; o `C28` não se move. **A WO não toca `src/`**: `node build.js` → `index.html` **846.128 bytes**, idêntico; `node validate.js index.html` → **18/18 · 102/102 · 0 erros**, o `C102` no log com `[ok (N=18, 6 caminhos meta/ conferidos)]`. `git diff` = `meta/CEREBRO.md` + `BUILD.md` + `validate.js` + a WO (nova), aditivo + as sete substituições previstas, sem remoção acidental. Commit `5076e8a`, empurrado (`d3e633b..5076e8a  main -> main`).

**Registrado, fora de escopo (os três primeiros pedem DECISÃO do dono, como a WO fixou):** candidato 1 — a skill `wrap` **gerada** manda escrever `logs/AAAA-MM-DD.md` e a **instalada** não, com `logs/` parado em 2026-09-02 e a frase de `validate.js:1082` («as skills mandam criar log e relatorio») desatualizada; candidato 6 — o invariante CRLF do `src/index.template.html` vem do `core.autocrlf` da máquina, não do repositório (`.gitattributes` não fixa nada), e a skill `apply-wo` afirma o CRLF como fato; candidato 4 — quatro referências a `meta/analises/*.md` inexistentes, pedindo allowlist comentada; a IDEA-141 do mapsmith («ao contornar uma restrição do produto, escreva por que ela existe»). Também fora daqui e para a raia de planejamento: o `## 🧪 Validação (regra dura: NUNCA publicar sem 17/17 e 0 erros)` no `meta/STATUS.md` — prosa de curadoria com a contagem antiga, que o `C102` não alcança (só lê CEREBRO e BUILD).

---

## D-147 — O CRLF do template ganha lastro no `.gitattributes`, e a skill `apply-wo` passa a dizer o que o git responde; entra o `C103` (wo0118)

**Data:** 2026-09-09 · **Base:** candidato 6 da exploração que o Code rodou no repositório inteiro (`260908-1009-code-kcm-explore-repo-inteiro.txt`), com decisão do dono de **fazer os dois lados** — o atributo e a skill. · **Cria** o `C103` (total **102 → 103**); não muda `src/`, `index.html` idêntico em **846.128 bytes**. · **Fecha** o candidato 6 que a D-146 registrou como fora de escopo, pedindo decisão do dono.

**O CRLF do template era hábito da máquina, não regra do repositório.** `git ls-files --eol src/index.template.html` devolvia `i/lf w/crlf attr/text=auto`: o índice guarda LF (correto), a árvore do dono tem CRLF, e o repositório não mandava nada — quem decidia era o `core.autocrlf` da máquina. Dezenas de WOs repetem «template ALL-CRLF» e calibram âncoras multi-linha nisso; um clone com `core.autocrlf=false` traria o template em LF e essas âncoras parariam de casar **sem aviso nenhum**. O precedente do custo já existe: na wo0112 um `sed -i` converteu o arquivo inteiro para LF e o único sinal foi a queda de tamanho do `index.html`.

**A operação é segura e não exige `--renormalize`.** O git já guarda LF no índice (`i/lf`); com `text eol=crlf` o que muda é só o que o checkout escreve na árvore. Nenhum arquivo do repositório é reescrito em massa. Depois da mudança o `git ls-files --eol` passa a devolver `i/lf w/crlf attr/text=crlf` — só o terceiro campo muda, e `i/lf` continua sendo o valor certo (não «consertar»). O `.gitattributes` já tinha precedente de fixar fim de linha por arquivo — `.githooks/pre-commit text eol=lf` (D-075) e `*.sh text eol=lf` —; a linha nova segue o mesmo padrão com o sinal invertido.

**A skill passa a dizer o que o git responde.** O item «Fim de linha por arquivo» da `apply-wo` afirmava o CRLF como fato, sem dizer que é da **árvore de trabalho** nem de onde vem a garantia. Agora diz: CRLF na árvore, garantido pelo `.gitattributes` e não pela configuração da máquina; LF para `validate.js`, `build.js`, os `src/niches/*.js` **e todo `.md`**; e explica que no índice o template é LF (`git ls-files --eol` → `i/lf w/crlf attr/text=crlf`), para quem comparar os dois números não achar um defeito onde não há.

**O `C103` guarda as duas pontas, com regex literal.** Deriva a raiz do repo de `path.dirname(path.resolve(path))`, confere que o `.gitattributes` fixa `src/index.template.html text eol=crlf` (`/^src\/index\.template\.html\s+text\s+eol=crlf\s*$/m` — barras **simples** porque é literal e não concatenada, ao contrário do `C102`; os dois estilos ficam como estão) e que a `SKILL.md` diz «árvore de trabalho» com a garantia creditada ao `.gitattributes`. **Dois pares negativos medidos** (sandbox reversível, restaurado por cópia de backup): remover a linha do `.gitattributes` → **VERMELHO no `C103`** («.gitattributes nao fixa `src/index.template.html text eol=crlf`: o CRLF que as skills assumem volta a depender do core.autocrlf da maquina»); trocar «árvore de trabalho» na skill → **VERMELHO no `C103`** («a skill apply-wo afirma o CRLF sem dizer que e da arvore de trabalho e de onde vem a garantia: o git responde i/lf e a skill diz CRLF, e as duas frases precisam concordar»).

**Corrige de passagem a justificativa do `Write` no allow.** `validate.js:1082` dizia «as skills mandam criar log e relatorio» — a skill `wrap` **instalada** não manda criar log (é o candidato 1, ainda aberto). A frase passa a «a skill wrap grava o relatorio em arquivo», que é verdade hoje, sem decidir nada sobre o log.

**Custo de teto: não se aplica** — nada entra em `buildInstr` nem no CEREBRO; o `C28` não se move. **A WO não toca `src/`**: `node build.js` → `index.html` **846.128 bytes**, idêntico (prova de que o fim de linha do template não mudou de verdade); `node validate.js index.html` → **18/18 · 103/103 · 0 erros**, o `C103` no log com `[ok (eol do template com lastro em .gitattributes)]`. `git ls-files --eol src/index.template.html` → `i/lf w/crlf attr/text eol=crlf`; `w/` já veio `crlf`, então não precisou `git checkout --`, e `git status` não acusa o template. `git diff` = `.gitattributes` + `.claude/skills/apply-wo/SKILL.md` + `validate.js` + a WO (nova), aditivo + as substituições previstas, sem remoção acidental (o aviso «LF will be replaced by CRLF» do `git diff` é ruído padrão do `core.autocrlf=true` no Windows — `git ls-files --eol` confirma `i/lf w/lf` nos três arquivos LF tocados). Commit `fcff0fc`, empurrado (`d7202de..fcff0fc  main -> main`).

**Medição pedida pela WO (candidato 4), rodada com o disco em mãos, nada consertado.** `git grep -hoE 'meta/(analises|workorders)/[0-9A-Za-z_.-]+\.md' | sort -u` + teste de existência devolve **7 `MISS`**: quatro são análises `.md` realmente ausentes do repo — `260718-ANALISE-i-N7-SDD-NICHOS-CODIGO.md` (a D-087 já registra que não foi recriada; citada em `DECISIONS.md:1054/1108`, `IDEAS.md:224`, `STATUS.md:182`, `260726-wo0054-sdd-leve.md:9`), `260728-ANALISE-bloco-gerenciado-vs-manual.md` (`260729-wo0067-…:15`, anterior à pasta `analises/`) e as duas `260810-ANALISE-…` (`fatiamento-no-formato-do-pacote` e `o-instrumento-mede-o-que-e-facil`), citadas por análises-irmãs do próprio KCM (`260811-…:45` e `:6`), **não «do mapsmith»** como a WO supôs; três são falsos-caminhos — `meta/workorders/AAMMDD-woNNNN-desc.md` (placeholder do `_TEMPLATE.md`), `_GUIA-doc-por-spec.md` (renomeado para `_GUIA-doc-por-wo.md` na wo0065) e `_GUIA.md` (string de exemplo num `L.push()`). A segunda metade do `C103` — varrer esses caminhos numa allowlist comentada caso a caso — fica para quando o dono devolver a lista, como a WO fixou.

**Registrado, fora de escopo (como a WO fixou):** o **log diário** (candidato 1) — a resposta muda o que se escreve: cláusula no `C43` + texto na skill instalada se «sim», linha declarando divergência de propósito se «não»; decisão do dono, custo no fluxo dele; `git add --renormalize` (desnecessário e arriscado — o índice já está LF); fixar fim de linha para outros arquivos (todos os demais são LF e já se comportam).

---

## D-148 — O log do dia volta a ser escrito na casa, e o `C103` ganha a segunda metade: caminho de análise ou WO citado tem de existir (wo0119)

**Data:** 2026-09-10 (a WO é datada de 2026-09-09) · **Base:** decisão do dono — **manter o log**, com a razão escrita: *é um documento técnico para o humano ler depois* —, mais a lista de 7 `MISS` que a raia de execução levantou no relatório `260909-1231-code-kcm-apply-wo0118.txt` (com o disco em mãos). · **Não cria check** (total **103/103**): o `C43` vai de 24 a **25 cláusulas** e o `C103` ganha a segunda metade. Não toca `src/`; `index.html` idêntico em **846.128 bytes**. · **Fecha** o candidato 1 que a D-146 e a D-147 registraram como fora de escopo, pedindo decisão do dono; **completa** a segunda metade do `C103` que a D-147 deixou pendente da medição.

**A skill `wrap` gerada manda criar `logs/AAAA-MM-DD.md` desde sempre (o `C47` confere isso no gerado); a instalada na casa não mandava.** Um vão exato: a `SKILL.md` de `wrap` tinha 24 cláusulas e nenhuma sobre o log, e por isso `logs/` parou em **2026-09-02** enquanto **oito sessões** (wo0108 a wo0117) fecharam com `/wrap`. O dono já havia recebido a recomendação contrária — «ninguém sentiu falta» —, e a rejeitou com o argumento que se sustenta: o log é para ser lido **no futuro**, não agora, e é a única peça do ciclo que **não sobe ao mount** (o que não for escrito lá só existe nos `.txt` da pasta-pai, que ninguém varre). Entra o bullet «Escreva o log do dia» antes de «Relate:» na skill instalada, e a **25ª cláusula do `C43`** (`["wrap: log do dia", /logs\/AAAA-MM-DD\.md/, ["wrap"]]`).

**O hiato de 03 a 08 de setembro fica DECLARADO, não reconstruído.** Escrever hoje seis arquivos datados de 03 a 08 produziria registro que **parece contemporâneo e não é** — exatamente o que a regra dura proíbe quando manda não reconstruir análise por memória. O log do dia da aplicação ganha **uma** seção de hiato, apontando onde o registro daqueles dias realmente está (`meta/STATUS.md` e os `.txt` da pasta-pai) e colando a saída real de `git log --since=2026-09-03 --until=2026-09-08` (16 commits). *Ausência declarada é dado; ausência preenchida por memória é ficção com carimbo de data.*

**O `C103` ganha a segunda metade, com a allowlist levantada pela execução — não inventada.** Varre `meta/CEREBRO.md`, `meta/DECISIONS.md`, `meta/IDEAS.md`, `meta/ROADMAP.md`, `meta/STATUS.md` e `CLAUDE.md` por citações `meta/(analises|workorders)/[0-9A-Za-z_.-]+\.md` (regex **literal**, barras simples, como o resto do `C103` e ao contrário do `C102`) e exige que cada uma exista no disco. `PERDOADOS` tem **cinco** casos históricos comentados um a um: `260718-ANALISE-i-N7-SDD-NICHOS-CODIGO.md` (a D-087 já registra em prosa que não foi recriada), `260728-ANALISE-bloco-gerenciado-vs-manual.md` (citada na wo0067, anterior à pasta `analises/`), as duas `260810-ANALISE-…` (citadas por análises-irmãs **do próprio KCM**, `260811-*` — **não «do mapsmith»**, correção da suposição da wo0118: atribuição errada num comentário de allowlist envelhece pior que a ausência que ela perdoa) e `meta/workorders/_GUIA.md` (renomeado, citação só em WO histórica). Os placeholders (`AAMMDD-woNNNN-desc.md`, `_GUIA-doc-por-spec.md`) são filtrados **por forma** (`/AAMMDD|NNNN|<|assunto|doc-por-spec/`), não por lista — allowlist é para caso concreto, filtro é para classe. Caminho fora da allowlist manda **PARAR e reportar**, não ampliar.

**Dois pares negativos medidos** (sandbox reversível, restaurado): quebrar o token `logs/AAAA-MM-DD.md` na skill instalada → **VERMELHO no `C43`** («a superficie INSTALADA `.claude/skills/wrap/SKILL.md` nao tem a clausula 'wrap: log do dia' que o kit publica»); renomear temporariamente `meta/analises/260713-ANALISE-NICHO-CARREIRA.md` → **VERMELHO no `C103`**, nomeando o caminho em `meta/DECISIONS.md` e `meta/STATUS.md`.
...
## FIX-038 — O fecho da wo0126 sobrescreveu a «Última sessão» da wo0125 em vez de rebaixá-la

**Data:** 2026-09-15 · **Base:** conferência do `git show` nos commits `a02e16f` (fecho da wo0125) e `470462d` (fecho da wo0126) durante o `/wrap` desta sessão.

**O defeito.** O fecho da wo0125 seguiu a forma correta: inseriu a nova entrada sob `## 💬 Última sessão (2026-09-14)` como bullet novo e empurrou a entrada anterior (wo0124) para baixo, sob um novo cabeçalho `## 💬 Sessão anterior`. O fecho seguinte (wo0126) não repetiu o padrão: trocou a data do cabeçalho para 2026-09-15 e **substituiu o bullet da wo0125 pelo da wo0126 no mesmo lugar**, sem rebaixá-lo antes — um `git diff` de 2 linhas onde deveria haver uma inserção. O conteúdo da wo0125 saiu do `meta/STATUS.md` inteiramente, sobrevivendo só no histórico do git. Fere diretamente o `CLAUDE.md` («append-only, não reescreve» para STATUS/DECISIONS/logs) e a própria instrução do `/wrap` («append na "Última sessão"»).

**A correção.** Restaurado nesta sessão: o bullet da wo0125 recuperado de `git show a02e16f:meta/STATUS.md` e reinserido como `## 💬 Sessão anterior (2026-09-14 — v1.122.0)`, entre a entrada da wo0126 (agora corretamente rebaixada) e a da wo0124 (que nunca saiu do lugar). Nenhum conteúdo foi perdido; a ordem cronológica das sessões está integral de novo.

**A lição de método.** O padrão «Última sessão» fixo + empurrar o antigo para «Sessão anterior» só se sustenta se **todo** fecho fizer as duas metades — inserir o novo E rebaixar o velho. Fazer só a primeira metade (sobrescrever no lugar) parece idêntico no resultado imediato (a seção "Última sessão" está atualizada) e só se revela como perda ao comparar contra o commit anterior — exatamente o tipo de defeito que o item **1b** do `/wrap` (conferir o relatório anterior contra o git) existe para pegar, mas que **1b** não cobre hoje: ele confere o que o relatório *afirma*, não a forma estrutural do arquivo que o fecho edita. Fica como achado para o chat considerar: o `/wrap` poderia conferir, antes de editar `STATUS.md`, que o bloco anterior ainda está presente em algum lugar do arquivo depois da edição.
