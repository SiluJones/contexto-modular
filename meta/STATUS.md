...
> Versão atual: **v1.56.0**. Teste: **17/17 nichos, 35/35 checagens, 0 erros JS** + integridade dos chips (FIX-004) + smoke/round-trip do HUB + G6 (skills-pack) + G7 (modo Code, migrado pro workmode) + G8 (selos de estado, agora lendo `WORK_MODES`). `N[narrative]` em 6688/6900. (Detalhe do método no CONTEXT §3.)
> **i-N36 fase "topbar inteiro" parte 1 aplicada** (spec0030: os 3 modos voltam ao topbar como botões-toggle coloridos, sticky de graça; painel «Modo de trabalho» e selos perto da saída saem). Resta da i-N36 o cluster de ação + modal (spec0031) e o atualizar (spec0032) + o encaixe do atualizador i-N40. Próximas frentes: spec0031, spec0032, i-N38 (hook de pre-commit rodando o harness), i-N39 (`/check-spec`).
