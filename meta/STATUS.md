...
> Versão atual: **v1.55.0**. Teste: **17/17 nichos, 35/35 checagens, 0 erros JS** + integridade dos chips (FIX-004) + smoke/round-trip do HUB + G6 (skills-pack) + G7 (modo Code, migrado pro workmode) + G8 (selos de estado). `N[narrative]` em 6688/6900. (Detalhe do método no CONTEXT §3.)
> **i-N36 fase C FECHADA** (spec0028 + spec0029: os 3 modos saem do topbar pro painel «Modo de trabalho» + selos de estado multicanal perto da saída). Resta da i-N36 só a fase "topbar inteiro" (adiada) + o encaixe do atualizador i-N40. Próximas frentes: i-N38 (hook de pre-commit rodando o harness), i-N39 (`/check-spec`), i-N40 (Modo Atualização, analisada/a escopar).
