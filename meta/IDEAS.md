...
## i-N54 — Auditar quais outros refinamentos universais não alcançaram os nichos com template próprio — ABERTA (wo0076)
**2026-08-02.** `dev` e `brainstorm` redefinem trechos universais (`IDEAS.md`, `STATUS.md`/`CONTEXT.md`) em vez de herdá-los — e três levas seguidas encontraram refinamento que ficou só no universal, sem alcançar o override: a wo0075/D-109 achou dois casos no `IDEAS.md` (as seções «Feedback para o Kit/ASU»); esta wo0076/D-110 achou um no `STATUS.md`/`CONTEXT.md` (`Arquivos Críticos`). Ninguém varreu o resto — `DECISIONS.md`, `CONTEXT.md`, `LOG-TEMPLATE`, prompts próprios — para saber se há mais casos escondidos. **Gatilho de volta:** no terceiro caso encontrado (este é o segundo — wo0075 conta como um, ainda que com dois itens), ou antes de qualquer leva futura que refine um documento universal, auditar a lista completa de trechos que `dev`/`brainstorm` sobrescrevem contra a versão universal correspondente.
