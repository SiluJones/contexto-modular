...
### 2026-07-26 — Correção pendente: bloco de fecho da wo0058 saiu com ordem/rótulos alterados
A wo0058 (D-092) implementou o bloco de fecho de turno, mas a **ordem e os rótulos** entregues divergem do formato que o usuário aprovou. Correto: **Próximo** (com heading) → divisor → **Estado** → **Arquivar / Manter** (rótulos assim, não «Notas») → **Config recomendada** (uma linha por raia, em lista) → **Handoff** ao final. Corrigir **depois da WO-C**, e **apresentar a estrutura antes de implementar**. Avaliar também deixar o protocolo **personalizável pelo próprio projeto** (cada chat ajusta as linhas conforme a própria produtividade, como manda o auto-refino da wo0055).