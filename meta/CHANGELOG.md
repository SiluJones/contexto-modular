...
- **Seção "Feedback para o ASU"** no IDEAS gerado, logo abaixo de "Feedback para o Kit".
...
### 1. IDEAS universal — fim do "faltou o IDEAS.md"
...
- No texto dos dois princípios usei os nomes **genéricos** que o kit gera (CONTEXT, STATUS, DECISIONS, CHANGELOG, IDEAS, ROADMAP), e **não** os nossos (DECISIONS/IDEAS), para casar com `HYGIENE_RULES` / `TRIGGERS_BASE` / `UPDATE_PROTOCOL`, que já usam os nomes genéricos no artefato gerado.
...
### Registros
- **D-018** (DECISIONS): o mount `/mnt/project/` é alimentado por **upload direto**, NÃO pelo conector do GitHub (que alimenta só o RAG/Conhecimento do Projeto); corrige a conclusão de D-016/v1.22.0. O mount chega achatado.
...
### Registros
- **D-017** (DECISIONS): refino P8/P11 + handoff multi-pasta + canal, com o achado do mount achatado e a recomendação do teste limpo.
...
### Registros
- **D-016** (DECISIONS): mount/ferramenta de código no protocolo + refino das diretrizes, com a verificação empírica e a reconciliação da divergência.
...
### Registros
- **D-015** (DECISIONS): protocolo de transferência (contexto vs. RAG), com fundamento técnico.
- **i-N9** (IDEAS): implementada. **i-N10** semeada (carimbo de versão automático nos downloads).
...
### Nicho brainstorm v2
- **7 templates**: TEMA, IDEAS, MAPA, FILTROS, STATUS, LOG-TEMPLATE (núcleo) + **DECISIONS** (opcional, quando alimenta escolhas reais).
- **IDEAS.md** reestruturado em ativas / banho-maria / descartadas-com-razão / sementes — "nada se perde" (a descartada pode ressuscitar noutro contexto).
...
### Nicho business v2
- **7 templates** (antes 6): CONTEXTO, OBJETIVOS, ANALISE, DECISIONS, STATUS, LOG-TEMPLATE (núcleo) + **MODELO-FINANCEIRO** (opcional, unit economics).
...
- **7 templates** (antes 6): PRODUTO, PERSONAS, JORNADAS, DECISIONS, STATUS, LOG-TEMPLATE (núcleo) + **EXPERIMENTOS** (opcional, validação por hipótese).
...
- **DECISIONS.md** com métrica esperada e hipóteses/riscos por decisão.
...
- **8 templates** (antes 6): TEMA, FONTES, HIPOTESES, SINTESE, STATUS, LOG-TEMPLATE (núcleo) + **GLOSSARY** e **HISTORY** (opcionais).
...
- **Gatilhos próprios** (triggersExtra): fonte verificada → FONTES; hipótese muda status → HIPOTESES; avanço → SINTESE; conceito definido → SINTESE/GLOSSARY.
...
- **9 templates** (antes 7): BIBLIA, PERSONAGENS, ENREDO, VOZ, CONTINUIDADE, STATUS, LOG-TEMPLATE (núcleo) + **CRONOLOGIA** e **GLOSSARY** (opcionais).
...
- **9 templates** (antes 7): PROJETO, CLIENTE, MARCA, REFERENCIAS, DECISIONS, REVISOES, STATUS, LOG-TEMPLATE (núcleo) + **PRODUCAO** (opcional, pré-impressão).
...
- **Gatilhos próprios** (triggersExtra): conceito definido → DECISIONS+MARCA; versão entregue → REVISOES+STATUS; decisão de cor/fonte → MARCA+DECISIONS; peça indo p/ impressão → checklist PRODUCAO; pedido extra → 'Yes-and'.
...
- **3 arquivos opcionais** (além dos 6 do núcleo): `ROADMAP.md` (plano em fases), `GLOSSARY.md` (termos do projeto), `HISTORY.md` (conhecimento consolidado de fases antigas, lido sob demanda). Total: 9 templates.
...
### Fundamento técnico (ver DECISIONS D-012)
...
Itens em `meta/IDEAS.md` arquivados como "evolução natural":
