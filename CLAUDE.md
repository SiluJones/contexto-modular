...
Grave o MESMO relatório também em `../AAMMDD-HHMM-code-kcm-<ato>-<alvo>.txt` (pasta-PAI do repo, fora do versionamento) — `<ato>` do vocabulário fechado `apply` · `wrap` · `probe` · `explore`, `<alvo>` = a WO ou duas ou três palavras com hífen — sempre, sem eu pedir, tanto em `/apply-wo` quanto em `/wrap`. Se a escrita for negada, DIGA e siga: o relatório no chat continua sendo a entrega. **Para desligar:** apague este parágrafo.
